    ...
    std::vector<luint_t>& roots)
{
    ...
    for(luint_t c = 0; c < num_components; ++c)
    {
        ...
        const luint_t c_k =
            std::max((luint_t) 1, (luint_t) std::floor(k *
...
sample_phase_I()
{
    std::mt19937 rnd_qu(this->m_rnd_dev());
        ...
        [&](const tbb::blocked_range<luint_t>& r)
        {
            ...
            for(luint_t ix = r.begin(); ix != r.end(); ++ix)
            {
                ...
                if(!ACYCLIC || m_markers[q_node] < 2)
                {
                    ...
                    const luint_t o_node = buf[d(rnd_gen)];
